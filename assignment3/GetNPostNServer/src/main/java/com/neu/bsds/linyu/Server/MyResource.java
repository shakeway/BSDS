...
import com.neu.bsds.linyu.Server.Cache.MessageCache;
...
public class MyResource {
    ...
    public SummaryRideData getData(@PathParam("skierID") String skierID, @PathParam("dayNum") String dayNum) throws SQLException {
        return LogServerMetriccsNGetData(skierID, dayNum);
    }

    private SummaryRideData LogServerMetriccsNGetData(String skierID, String dayNum) {
        ...
        MessageCache.addGetMessage(message);
        return res;
    }
    ...
    public String postData(SingleRideData sd) throws SQLException {
        //postAndUpdate(sd);
        //postWithCache(sd);
        LogServerMetriccsNPostData(sd);
        return sd.toString();
    }

    private void LogServerMetriccsNPostData(SingleRideData sd) {
        long startTime = System.currentTimeMillis();
        int errorNum = 0;
        try {
            postWithCache(sd);
        ...
        MessageCache.addPostMessage(message);
    }
