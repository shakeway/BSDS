...
try:
    ...
    print("total number of request is:", total_request_num[0])

    cur.execute("select sum(errorNum) from server_table")
    error_num = cur.fetchone();
    print("total number of error is:", error_num[0])

    print("---------------- statics of post --------------------")
